...
# Download 2023 Census data using caching function
raw_data <- cache_census_api_call(
  type = "pums",
  year = 2023,
  survey = "acs5",
  state = "TX",
...
analysis_data <- raw_data |>
  dplyr::mutate(
    ...
    education_number = base::suppressWarnings(base::as.numeric(SCHL)),
    has_bachelors_or_higher = base::ifelse(education_number >= 21, 1, 0),
    has_post_graduate_degree = base::ifelse(education_number >= 22, 1, 0),
    
    # Family and work characteristics
    age_category = base::cut(AGEP, breaks = c(0, 25, 35, 45, 55, 65, 100)),
    is_married = base::ifelse(MAR == "1", 1, 0),
    never_been_married = base::ifelse(MAR == "5", 1, 0),
    works_full_time = base::ifelse(WKHP >= 35, 1, 0),
    ...
    per_capita_income = base::ifelse(NP > 0 & !base::is.na(HINCP) & !base::is.na(NP), HINCP / NP, NA)
  ) |>
  dplyr::filter(
    AGEP >= 25 & AGEP <= 65,        # Working age adults only
    racial_group != "Other",         # Focus on main racial groups
    !base::is.na(education_number),        # Remove missing education data
    !base::is.na(per_capita_income),       # Remove missing income data
...
**Sample:** `r base::nrow(analysis_data)` adults between ages 25-65 with complete demographic and income data.

## Step 1: Raw Patterns by Race and Ethnicity

```{r}
#| label: raw-patterns-data
#| echo: true
...
raw_patterns <- analysis_data |>
  ...
  dplyr::summarise(
    `Number of People` = dplyr::n(),
    `Pct with Bachelor's Degree or Higher` = scales::percent(base::mean(has_bachelors_or_higher, na.rm = TRUE), accuracy = 1),
    `Pct Currently Married` = scales::percent(base::mean(is_married, na.rm = TRUE), accuracy = 1),
    `Pct with Children at Home` = scales::percent(base::mean(has_children_at_home, na.rm = TRUE), accuracy = 1),
    `Pct Working Full-Time` = scales::percent(base::mean(works_full_time, na.rm = TRUE), accuracy = 1),
    `Median Per Capita Income` = scales::dollar(stats::median(per_capita_income, na.rm = TRUE)),
    `Mean Per Capita Income` = scales::dollar(base::round(base::mean(per_capita_income, na.rm = TRUE), 0)),
    .groups = "drop"
  ) |>
  dplyr::arrange(dplyr::desc(`Median Per Capita Income`))

# Test if raw differences are statistically significant
raw_income_test <- stats::aov(per_capita_income ~ racial_group, data = analysis_data)
raw_income_results <- base::summary(raw_income_test)

raw_significance <- base::data.frame(
  Test = "Raw Per Capita Income Differences by Race",
  F_Statistic = base::round(raw_income_results[[1]]$`F value`[1], 2),
  P_Value = base::format.pval(raw_income_results[[1]]$`Pr(>F)`[1], digits = 3),
  Conclusion = base::ifelse(raw_income_results[[1]]$`Pr(>F)`[1] < 0.05, 
...
```{r}
#| label: raw-patterns-pdf
#| echo: true

flextable::flextable(raw_patterns) |>
  flextable::set_caption(caption = "Step 1: Raw Demographic and Economic Patterns by Race/Ethnicity (No Controls)") |>
  flextable::add_header_lines(values = "Raw Demographic and Economic Patterns") |>
...
::: {.content-visible unless-format="html"}
```{r}
#| label: raw-significance-pdf
#| echo: true

flextable::flextable(raw_significance) |>
  flextable::set_caption(caption = "Statistical Test: Are Raw Racial Income Differences Real?") |>
  flextable::add_header_lines(values = "Raw Income Differences Statistical Test") |>
  flextable::color(i = 1, color = "blue", part = "header") |>
  flextable::italic(i = 1, part = "header") |>
  flextable::align(i = 1, align = "center", part = "header") |>
  flextable::fontsize(i = 1, size = 14, part = "header") |>
...
criteria_table <- base::data.frame(
...
::: {.content-visible when-format="html"}
```{r}
#| label: performance-summary-html
#| echo: true
...
::: {.content-visible unless-format="html"}
```{r}
#| label: performance-summary-pdf
#| echo: true

flextable::flextable(performance_summary) |>
  flextable::set_caption(caption = "Step 2: Number of High vs Low Performers by Race (Adults Ages 30-50)") |>
  flextable::add_header_lines(values = "High vs Low Performers by Race") |>
  flextable::color(i = 1, color = "blue", part = "header") |>
  flextable::italic(i = 1, part = "header") |>
  flextable::align(i = 1, align = "center", part = "header") |>
  flextable::fontsize(i = 1, size = 14, part = "header") |>
...
::: {.content-visible unless-format="html"}
```{r}
#| label: criteria-table-pdf
#| echo: true

flextable::flextable(criteria_table) |>
  flextable::set_caption(caption = "Clear Definition of High vs Low Performers") |>
  flextable::add_header_lines(values = "Performance Level Definitions") |>
  flextable::color(i = 1, color = "blue", part = "header") |>
  flextable::italic(i = 1, part = "header") |>
  flextable::align(i = 1, align = "center", part = "header") |>
  flextable::fontsize(i = 1, size = 14, part = "header") |>
...
```{r}
#| label: high-performers-across-races-data
#| echo: true
...
high_performers_comparison <- performance_data |>
  ...
  dplyr::group_by(racial_group) |>
  dplyr::summarise(
    `Number of High Performers` = dplyr::n(),
    `Median Per Capita Income` = stats::median(per_capita_income, na.rm = TRUE),
    `Mean Per Capita Income` = base::round(base::mean(per_capita_income, na.rm = TRUE), 0),
    `Pct with Post-Graduate Degrees` = scales::percent(base::mean(has_post_graduate_degree, na.rm = TRUE), accuracy = 1),
    `Pct Currently Married` = scales::percent(base::mean(is_married, na.rm = TRUE), accuracy = 1),
    `Pct Working Full-Time` = scales::percent(base::mean(works_full_time, na.rm = TRUE), accuracy = 1),
    `Pct with Children at Home` = scales::percent(base::mean(has_children_at_home, na.rm = TRUE), accuracy = 1),
    .groups = "drop"
...
high_performer_income_test <- stats::aov(per_capita_income ~ racial_group, data = high_performer_data)
high_performer_results <- base::summary(high_performer_income_test)

# Calculate the range of differences between high performers across races
high_performer_range <- base::max(high_performers_comparison$`Median Per Capita Income`) - 
                       base::min(high_performers_comparison$`Median Per Capita Income`)

high_performer_significance <- base::data.frame(
  Test = "Per Capita Income Differences Among High Performers Across Races",
  F_Statistic = base::round(high_performer_results[[1]]$`F value`[1], 2),
  P_Value = base::format.pval(high_performer_results[[1]]$`Pr(>F)`[1], digits = 3),
  Income_Range = scales::dollar(high_performer_range),
  Conclusion = base::ifelse(high_performer_results[[1]]$`Pr(>F)`[1] < 0.05, 
...
::: {.content-visible when-format="html"}
```{r}
#| label: high-performers-comparison-html
#| echo: true
...
::: {.content-visible unless-format="html"}
```{r}
#| label: high-performers-comparison-pdf
#| echo: true

flextable::flextable(high_performers_comparison) |>
  flextable::set_caption(caption = "Step 3: High Performers Compared Across Racial Groups") |>
  flextable::add_header_lines(values = "High Performers Across Racial Groups") |>
  flextable::color(i = 1, color = "blue", part = "header") |>
  flextable::italic(i = 1, part = "header") |>
  flextable::align(i = 1, align = "center", part = "header") |>
  flextable::fontsize(i = 1, size = 14, part = "header") |>
  flextable::bg(i = 1, bg = "white", part = "header") |>
  flextable::bg(i = 2, bg = "palegreen", part = "header") |>
  flextable::colformat_double(j = c("Median Per Capita Income", "Mean Per Capita Income"), big.mark = ",", prefix = "$", digits = 0) |>
  ftExtra::colformat_md() |>
  flextable::autofit()
```
:::
...
::: {.content-visible when-format="html"}
```{r}
#| label: high-performer-significance-html
#| echo: true
...
::: {.content-visible unless-format="html"}
```{r}
#| label: high-performer-significance-pdf
#| echo: true

flextable::flextable(high_performer_significance) |>
  flextable::set_caption(caption = "Statistical Test: Are High Performers Different Across Racial Groups?") |>
  flextable::add_header_lines(values = "High Performer Differences Statistical Test") |>
  flextable::color(i = 1, color = "blue", part = "header") |>
  flextable::italic(i = 1, part = "header") |>
  flextable::align(i = 1, align = "center", part = "header") |>
  flextable::fontsize(i = 1, size = 14, part = "header") |>
...
```{r}
#| label: within-group-comparison-data
#| echo: true
...
within_group_comparison <- performance_data |>
  ...
  dplyr::summarise(
    ...
    `Median Per Capita Income` = stats::median(per_capita_income, na.rm = TRUE),
    `Mean Per Capita Income` = base::round(base::mean(per_capita_income, na.rm = TRUE), 0),
    `Pct Currently Married` = base::mean(is_married, na.rm = TRUE),
    `Pct Working Full-Time` = base::mean(works_full_time, na.rm = TRUE),
    `Pct with Children at Home` = base::mean(has_children_at_home, na.rm = TRUE),
    .groups = "drop"
  )
...
avg_within_group_income_gap <- base::mean(within_group_table$`Income Gap (High - Low)`, na.rm = TRUE)
avg_within_group_marriage_gap <- base::mean(within_group_table$`Marriage Gap (High - Low)`, na.rm = TRUE)
```

::: {.content-visible when-format="html"}
```{r}
#| label: within-group-table-html
#| echo: true
...
::: {.content-visible unless-format="html"}
```{r}
#| label: within-group-table-pdf
#| echo: true

flextable::flextable(within_group_table) |>
  flextable::set_caption(caption = "Step 4: High vs Low Performers Within Each Racial Group") |>
  flextable::add_header_lines(values = "Within-Group Comparison") |>
  flextable::color(i = 1, color = "blue", part = "header") |>
  flextable::italic(i = 1, part = "header") |>
  flextable::align(i = 1, align = "center", part = "header") |>
  flextable::fontsize(i = 1, size = 14, part = "header") |>
  flextable::bg(i = 1, bg = "white", part = "header") |>
  flextable::bg(i = 2, bg = "palegreen", part = "header") |>
  flextable::colformat_double(j = c("High Performer Income", "Low Performer Income", "Income Gap (High - Low)"), big.mark = ",", prefix = "$", digits = 0) |>
  flextable::colformat_double(j = "Marriage Gap (High - Low)", digits = 1, suffix = "%") |>
  ftExtra::colformat_md() |>
  flextable::autofit()
```
:::
...
key_finding_table <- base::data.frame(
  ...
  `Statistical Significance` = c(
    base::ifelse(high_performer_results[[1]]$`Pr(>F)`[1] < 0.05, "Significant", "Not Significant"),
    "Significant (large effect sizes)"
  ),
  `Interpretation` = c(
    base::ifelse(high_performer_results[[1]]$`Pr(>F)`[1] < 0.05, 
...
ratio_interpretation <- base::data.frame(
  `Comparison` = "Within-Group vs Between-Group Income Differences",
  `Ratio` = base::round(difference_ratio, 1),
  `Meaning` = base::paste0("Within-group differences are ", base::round(difference_ratio, 1), 
                    " times larger than between-group differences"),
  `Conclusion` = base::ifelse(difference_ratio > 2, 
...
::: {.content-visible unless-format="html"}
```{r}
#| label: key-finding-table-pdf
#| echo: true

flextable::flextable(key_finding_table) |>
  flextable::set_caption(caption = "KEY FINDING: Between vs Within Group Differences") |>
  flextable::add_header_lines(values = "Key Finding: Between vs Within Group Differences") |>
  flextable::color(i = 1, color = "blue", part = "header") |>
  flextable::italic(i = 1, part = "header") |>
  flextable::align(i = 1, align = "center", part = "header") |>
  flextable::fontsize(i = 1, size = 14, part = "header") |>
...
::: {.content-visible when-format="html"}
```{r}
#| label: ratio-interpretation-html
#| echo: true
...
::: {.content-visible unless-format="html"}
```{r}
#| label: ratio-interpretation-pdf
#| echo: true

flextable::flextable(ratio_interpretation) |>
  flextable::set_caption(caption = "How Much Bigger Are Within-Group vs Between-Group Differences?") |>
  flextable::add_header_lines(values = "Ratio Interpretation") |>
  flextable::color(i = 1, color = "blue", part = "header") |>
  flextable::italic(i = 1, part = "header") |>
  flextable::align(i = 1, align = "center", part = "header") |>
  flextable::fontsize(i = 1, size = 14, part = "header") |>
...
plot_data <- within_group_comparison |>
  ...
  dplyr::mutate(
    racial_group = base::factor(racial_group, 
                         levels = c("Asian (Non-Hispanic)", "White (Non-Hispanic)", 
                                   "Hispanic (Any Race)", "Black (Non-Hispanic)")),
    performance_level = base::factor(performance_level, levels = c("High Performer", "Low Performer"))
  )
...
comparison_plot <- plot_data |>
  ...
  ggplot2::scale_y_continuous(labels = scales::dollar_format(), 
                             limits = c(0, base::max(plot_data$`Median Per Capita Income`) * 1.1)) +
  ...
  ggplot2::labs(
    ...
    caption = base::paste0("Key Finding: Within-group differences (high vs low) are ", 
                    base::round(difference_ratio, 1), " times larger than between-group differences (across races)")
  ) +
...
base::print(comparison_plot)

# Add horizontal lines to show the difference ranges
comparison_plot_with_annotations <- comparison_plot +
  ggplot2::annotate("text", x = 2.5, y = base::max(plot_data$`Median Per Capita Income`) * 0.9,
           label = base::paste0("Between-group difference among high performers: $", 
                         scales::comma(between_group_gap_high_performers)),
           size = 4, fontface = "bold", color = "darkgreen") +
  ggplot2::annotate("text", x = 2.5, y = base::max(plot_data$`Median Per Capita Income`) * 0.8,
           label = base::paste0("Average within-group difference: $", 
                         scales::comma(base::round(within_group_gap_average, 0))),
           size = 4, fontface = "bold", color = "darkred")

base::print(comparison_plot_with_annotations)
```

## Statistical Analysis of Marriage Patterns

```{r}
#| label: marriage-analysis-data
#| echo: true
...
marriage_analysis <- performance_data |>
  ...
  dplyr::summarise(
    sample_size = dplyr::n(),
    marriage_rate = base::mean(is_married, na.rm = TRUE),
    .groups = "drop"
  ) |>
  dplyr::mutate(
...
marriage_chi_test <- stats::chisq.test(base::table(marriage_test_data$racial_group, marriage_test_data$is_married))

marriage_test_results <- base::data.frame(
  Test = "Marriage Rate Differences Among High Performers",
  Chi_Square = base::round(marriage_chi_test$statistic, 2),
  P_Value = base::format.pval(marriage_chi_test$p.value, digits = 3),
  Conclusion = base::ifelse(marriage_chi_test$p.value < 0.05,
...
::: {.content-visible unless-format="html"}
```{r}
#| label: marriage-analysis-pdf
#| echo: true

flextable::flextable(marriage_analysis) |>
  flextable::set_caption(caption = "Marriage Rates Among High Performers by Race") |>
  flextable::add_header_lines(values = "Marriage Rates Among High Performers") |>
  flextable::color(i = 1, color = "blue", part = "header") |>
  flextable::italic(i = 1, part = "header") |>
  flextable::align(i = 1, align = "center", part = "header") |>
  flextable::fontsize(i = 1, size = 14, part = "header") |>
...
::: {.content-visible when-format="html"}
```{r}
#| label: marriage-test-results-html
#| echo: true
...
::: {.content-visible unless-format="html"}
```{r}
#| label: marriage-test-results-pdf
#| echo: true

flextable::flextable(marriage_test_results) |>
  flextable::set_caption(caption = "Statistical Test: Marriage Rate Differences Among High Performers") |>
  flextable::add_header_lines(values = "Marriage Rate Statistical Test") |>
  flextable::color(i = 1, color = "blue", part = "header") |>
  flextable::italic(i = 1, part = "header") |>
  flextable::align(i = 1, align = "center", part = "header") |>
  flextable::fontsize(i = 1, size = 14, part = "header") |>
...
- `r base::ifelse(high_performer_results[[1]]$'Pr(>F)'[1] < 0.05, base::paste0("Some statistically significant differences remain among high performers across races, but they are much smaller than within-group differences (range: $", scales::comma(high_performer_range), ")"), base::paste0("No statistically significant differences exist among high performers across races (income range: $", scales::comma(high_performer_range), ")"))`
...
- Average income difference between high and low performers within the same race: $`r scales::comma(base::round(within_group_gap_average, 0))`
- Income difference range among high performers across races: $`r scales::comma(high_performer_range)`
- **The within-group difference is `r base::round(difference_ratio, 1)` times larger than the between-group difference**
...
- F-statistic: `r base::round(high_performer_results[[1]]$'F value'[1], 2)`
- P-value: `r base::format.pval(high_performer_results[[1]]$'Pr(>F)'[1], digits = 3)`
...
- Average income gap: $`r scales::comma(base::round(within_group_gap_average, 0))`
...
### `r base::ifelse(high_performer_results[[1]]$'Pr(>F)'[1] < 0.05, "Remaining Racial Differences Among High Performers Are Small", "No Meaningful Racial Differences Among High Performers")`

`r base::ifelse(high_performer_results[[1]]$'Pr(>F)'[1] < 0.05, "While some statistically detectable differences remain among high performers across racial groups, these differences are much smaller than the differences between high and low performers within each group. This suggests that individual achievement largely levels the playing field.", "Among high-performing individuals, racial differences in economic outcomes become statistically non-significant. This suggests that educational achievement, stable marriages, and consistent work patterns create similar economic outcomes regardless of racial background.")`
