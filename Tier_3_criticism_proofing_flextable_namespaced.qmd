...
# Flextable built-in themes:
  ...
  # flextable::theme_trodplyr::n() |>  # 'DARK MODE' BLUE TEXT
...
# Download 2023 Census data with comprehensive defensive variables (with caching)
defensive_raw_data <- base::suppressMessages({
  ...
  dplyr::mutate(
    ...
    dplyr::across(c(SPORDER, SERIALNO, PUMA, STATE, RAC1P, HISP, SEX, MAR, ESR, HHT, SCHL, 
             OCCP, INDP, COW, CIT, NATIVITY, FOD1P, TEN, LANX, ENG), base::as.character),
    dplyr::across(c(WGTP, PWGTP, AGEP, HINCP, WAGP, WKHP, POVPIP, NP, YOEP, VALP, VEH), base::as.numeric)
  )
```
...
defensive_analysis_data <- defensive_raw_data |>
  dplyr::mutate(
    ...
    generational_cohort = dplyr::case_when(
      birth_year_estimated >= 1997 ~ "Gen_Z_1997_Plus",
      birth_year_estimated >= 1981 & birth_year_estimated <= 1996 ~ "Millennials_1981_1996",
      birth_year_estimated >= 1965 & birth_year_estimated <= 1980 ~ "Gen_X_1965_1980",
      birth_year_estimated >= 1946 & birth_year_estimated <= 1964 ~ "Boomers_1946_1964",
    ...
    geographic_area_type = dplyr::case_when(
      PUMA %in% c("05700") ~ "Suburban_Guadalupe_County",
      PUMA %in% c("05916") ~ "Suburban_Northeast_Bexar",
      PUMA %in% c("06100") ~ "Urban_Central_San_Antonio",
      PUMA %in% c("06200") ~ "Mixed_Density_South_Bexar",
      TRUE ~ "Other_Geographic_Area"
    ),
    
    # TIER 3 DEFENSIVE - Immigration timing for selection bias testing
    immigration_era_category = dplyr::case_when(
      base::is.na(YOEP) | CIT %in% c("1", "2", "3") ~ "Native_Born_US_Citizen",
      !base::is.na(YOEP) & YOEP >= 2010 ~ "Recent_Immigrant_2010_Plus",
      !base::is.na(YOEP) & YOEP >= 2000 & YOEP < 2010 ~ "Established_Immigrant_2000s",
      !base::is.na(YOEP) & YOEP < 2000 ~ "Long_Term_Immigrant_Pre_2000",
    ...
    occupation_major_category = dplyr::case_when(
      base::substr(OCCP, 1, 2) %in% c("11", "13", "15", "17", "19") ~ "Management_Professional_STEM",
      base::substr(OCCP, 1, 2) %in% c("21", "23", "25", "27", "29") ~ "Professional_Social_Services", 
      base::substr(OCCP, 1, 2) %in% c("31", "33", "35", "37", "39") ~ "Service_Occupations",
      base::substr(OCCP, 1, 2) %in% c("41", "43", "45", "47", "49") ~ "Sales_Office_Construction",
      base::substr(OCCP, 1, 2) %in% c("51", "53") ~ "Production_Transportation",
      base::is.na(OCCP) | OCCP == "" ~ "Not_Working_No_Occupation",
    ...
    educational_institution_proxy = dplyr::case_when(
      base::as.numeric(SCHL) < 16 ~ "High_School_Or_Less",
      base::as.numeric(SCHL) == 17 ~ "Some_College_No_Degree",
      base::as.numeric(SCHL) == 18 ~ "Associates_Degree_Community_College",
      base::as.numeric(SCHL) == 19 ~ "Associates_Degree_Academic", 
      base::as.numeric(SCHL) == 20 ~ "Some_College_One_Year",
      base::as.numeric(SCHL) == 21 ~ "Bachelors_Degree_Standard",
      base::as.numeric(SCHL) >= 22 ~ "Graduate_Professional_Degree",
    ...
    multiple_wealth_indicators = dplyr::case_when(
      TEN %in% c("1", "2") & !base::is.na(VEH) & base::as.numeric(VEH) >= 2 ~ "High_Wealth_Indicators",
      TEN %in% c("1", "2") & (base::is.na(VEH) | base::as.numeric(VEH) <= 1) ~ "Medium_Wealth_Homeowner",
      TEN == "3" & !base::is.na(VEH) & base::as.numeric(VEH) >= 2 ~ "Medium_Wealth_Renter",
      TEN == "3" & (base::is.na(VEH) | base::as.numeric(VEH) <= 1) ~ "Low_Wealth_Indicators",
      TRUE ~ "Unknown_Wealth_Status"
    ),
    
    # TIER 3 DEFENSIVE - Work stability for employment selection effects
    employment_stability_measure = dplyr::case_when(
      !base::is.na(WKHP) & WKHP >= 35 ~ "Stable_Full_Time_Employment",
      !base::is.na(WKHP) & WKHP >= 20 & WKHP < 35 ~ "Stable_Part_Time_Employment",
      !base::is.na(WKHP) & WKHP < 20 ~ "Limited_Hours_Employment",
      base::is.na(WKHP) | WKHP == 0 ~ "Not_Working_Recently",
      TRUE ~ "Unknown_Employment_Status"
    ),
    
    # Core economic measures for defensive testing
    education_level_defensive = base::suppressWarnings(base::as.numeric(SCHL)),
    has_bachelors_or_higher_defensive = base::ifelse(education_level_defensive >= 21, 1, 0),
    per_capita_household_income_defensive = base::ifelse(NP > 0 & !base::is.na(HINCP) & !base::is.na(NP), HINCP / NP, NA),
    individual_wage_income_defensive = base::ifelse(!base::is.na(WAGP) & WAGP > 0, WAGP, 0)
  ) |>
  dplyr::filter(
    AGEP >= 25 & AGEP <= 65,
    racial_category_defensive != "Other_Mixed",
    !base::is.na(education_level_defensive),
    !base::is.na(per_capita_household_income_defensive),
    per_capita_household_income_defensive > 0,
    per_capita_household_income_defensive < 500000
  ) |>
  dplyr::mutate(
...
defensive_data_summary <- base::data.frame(
  Total_Sample_Size = base::nrow(defensive_analysis_data),
  Geographic_Areas_Included = base::length(base::unique(defensive_analysis_data$geographic_area_type)),
  Generational_Cohorts = base::length(base::unique(defensive_analysis_data$generational_cohort)) - 1,
  Occupation_Categories = base::length(base::unique(defensive_analysis_data$occupation_major_category)) - 1,
  Defensive_Tests_Planned = "Age Cohorts, Geography, Immigration, Occupation, Education, Measurement"
)
```

::: {.content-visible when-format="html"}
```{r defensive-data-summary-dt}
#| label: defensive-data-summary-dt
...
```{r defensive-data-summary-ft}
#| label: defensive-data-summary-ft

flextable::flextable(defensive_data_summary) |>
  flextable::set_caption(caption = "Tier 3 Defensive Research Dataset Summary") |>
  flextable::add_header_lines(values = "Defensive Dataset Summary") |>
  flextable::color(i = 1, color = "blue", part = "header") |>
  flextable::italic(i = 1, part = "header") |>
  flextable::align(i = 1, align = "center", part = "header") |>
  flextable::fontsize(i = 1, size = 14, part = "header") |>
...
generational_patterns_by_cohort <- generational_analysis |>
  ...
  dplyr::summarise(
    sample_size = dplyr::n(),
    median_income = stats::median(per_capita_household_income_defensive, na.rm = TRUE),
    mean_income = base::mean(per_capita_household_income_defensive, na.rm = TRUE),
    .groups = "drop"
  ) |>
...
generational_between_group_analysis <- generational_patterns_by_cohort |>
  ...
  dplyr::summarise(
    racial_groups_present = dplyr::n(),
    income_range_between_races = base::max(median_income) - min(median_income),
    mean_income_level = base::mean(median_income),
    .groups = "drop"
  ) |>
...
generational_within_group_analysis <- generational_patterns_by_cohort |>
  ...
  dplyr::summarise(
    ...
    within_group_income_gap = base::ifelse(length(high_performer_income) > 0 & length(low_performer_income) > 0, 
                                    high_performer_income - low_performer_income, NA),
    .groups = "drop"
  ) |>
  dplyr::filter(!base::is.na(within_group_income_gap)) |>
  dplyr::group_by(generational_cohort) |>
  dplyr::summarise(
    average_within_group_gap = base::mean(within_group_income_gap, na.rm = TRUE),
    .groups = "drop"
  )
...
generational_cohort_results <- generational_between_group_analysis |>
  ...
  dplyr::mutate(
    within_to_between_ratio = base::round(average_within_group_gap / income_range_between_races, 1),
...
::: {.content-visible when-format="html"}
```{r generational-cohort-results-dt}
#| label: generational-cohort-results-dt
...
::: {.content-visible unless-format="html"}
```{r generational-cohort-results-ft}
#| label: generational-cohort-results-ft

flextable::flextable(generational_cohort_results) |>
  flextable::set_caption(caption = "Generational Cohort Analysis: Within-Group vs Between-Group Patterns") |>
  flextable::add_header_lines(values = "Generational Cohort Analysis") |>
  flextable::color(i = 1, color = "blue", part = "header") |>
  flextable::italic(i = 1, part = "header") |>
  flextable::align(i = 1, align = "center", part = "header") |>
  flextable::fontsize(i = 1, size = 14, part = "header") |>
  flextable::bg(i = 1, bg = "white", part = "header") |>
  flextable::bg(i = 2, bg = "palegreen", part = "header") |>
  flextable::colformat_double(j = c("income_range_between_races", "average_within_group_gap"), big.mark = ",", prefix = "$", digits = 0) |>
  ftExtra::colformat_md() |>
  flextable::autofit()
```
:::

```{r generational-consistency-test-data}
#| label: generational-consistency-test-data

# Test for consistent patterns across generations
generational_consistency_test <- base::data.frame(
  Criticism_Tested = "Results Only Apply to Certain Age Groups",
  Millennials_Pattern = base::ifelse(
  ...
  Defense_Verdict = base::ifelse(
    base::nrow(generational_cohort_results) >= 2 && 
    base::sum(base::grepl("Strong|Moderate", generational_cohort_results$pattern_strength)) >= 2,
...
::: {.content-visible when-format="html"}
```{r generational-consistency-test-dt}
#| label: generational-consistency-test-dt
...
::: {.content-visible unless-format="html"}
```{r generational-consistency-test-ft}
#| label: generational-consistency-test-ft

flextable::flextable(generational_consistency_test) |>
  flextable::set_caption(caption = "Age Cohort Defense: Generational Consistency Test Results") |>
  flextable::add_header_lines(values = "Generational Consistency Test") |>
  flextable::color(i = 1, color = "blue", part = "header") |>
  flextable::italic(i = 1, part = "header") |>
  flextable::align(i = 1, align = "center", part = "header") |>
  flextable::fontsize(i = 1, size = 14, part = "header") |>
...
geographic_patterns_by_area <- geographic_bias_analysis |>
  ...
  dplyr::summarise(
    sample_size = dplyr::n(),
    median_income = stats::median(per_capita_household_income_defensive, na.rm = TRUE),
    homeownership_rate = base::mean(ifelse(TEN %in% c("1", "2"), 1, 0), na.rm = TRUE),
    .groups = "drop"
  ) |>
...
::: {.content-visible when-format="html"}
```{r geographic-area-results-dt}
#| label: geographic-area-results-dt
...
::: {.content-visible unless-format="html"}
```{r geographic-area-results-ft}
#| label: geographic-area-results-ft

flextable::flextable(geographic_area_results) |>
  flextable::set_caption(caption = "Geographic Area Analysis: Urban vs Suburban Patterns") |>
  flextable::add_header_lines(values = "Geographic Area Analysis") |>
  flextable::color(i = 1, color = "blue", part = "header") |>
  flextable::italic(i = 1, part = "header") |>
  flextable::align(i = 1, align = "center", part = "header") |>
  flextable::fontsize(i = 1, size = 14, part = "header") |>
...
```{r geographic-consistency-test-data}
#| label: geographic-consistency-test-data

# Test for geographic consistency
geographic_consistency_test <- base::data.frame(
  Criticism_Tested = "Results Biased by Geographic Selection",
  Areas_Tested = base::ifelse(base::nrow(geographic_area_results) > 0,
                       base::paste(geographic_area_results$geographic_area_type, collapse = "; "),
                       "Insufficient Geographic Data"),
  Pattern_Consistency = base::ifelse(
    base::nrow(geographic_area_results) > 0 && base::sum(geographic_area_results$within_to_between_ratio > 1.5, na.rm = TRUE) >= 1,
    "CONSISTENT - Pattern holds across available geographic areas",
    "INSUFFICIENT DATA - Limited geographic variation for testing"
  ),
  Geographic_Range_Ratio = base::ifelse(base::nrow(geographic_area_results) > 0,
                                 base::paste0(base::round(base::min(geographic_area_results$within_to_between_ratio, na.rm = TRUE), 1), 
                                       " to ", base::round(base::max(geographic_area_results$within_to_between_ratio, na.rm = TRUE), 1)),
                                 "No Data Available"),
  Defense_Verdict = base::ifelse(
    base::nrow(geographic_area_results) > 0 && base::sum(geographic_area_results$within_to_between_ratio > 1.5, na.rm = TRUE) >= 1,
...
::: {.content-visible when-format="html"}
```{r geographic-consistency-test-dt}
#| label: geographic-consistency-test-dt
...
::: {.content-visible unless-format="html"}
```{r geographic-consistency-test-ft}
#| label: geographic-consistency-test-ft

flextable::flextable(geographic_consistency_test) |>
  flextable::set_caption(caption = "Geographic Bias Defense: Area Type Consistency Test Results") |>
  flextable::add_header_lines(values = "Geographic Consistency Test") |>
  flextable::color(i = 1, color = "blue", part = "header") |>
  flextable::italic(i = 1, part = "header") |>
  flextable::align(i = 1, align = "center", part = "header") |>
  flextable::fontsize(i = 1, size = 14, part = "header") |>
...
::: {.content-visible when-format="html"}
```{r occupation-data-check-dt}
#| label: occupation-data-check-dt

DT::datatable(occupation_data_check,
              caption = "DIAGNOSTIC: Occupation Data Available by Race",
              options = base::list(pageLength = 15, dom = 't'),
              rownames = FALSE)
```
:::

::: {.content-visible unless-format="html"}
```{r occupation-data-check-ft}
#| label: occupation-data-check-ft

flextable::flextable(occupation_data_check) |>
  flextable::set_caption(caption = "DIAGNOSTIC: Occupation Data Available by Race") |>
  flextable::add_header_lines(values = "Occupation Data Check") |>
  flextable::color(i = 1, color = "blue", part = "header") |>
  flextable::italic(i = 1, part = "header") |>
  flextable::align(i = 1, align = "center", part = "header") |>
  flextable::fontsize(i = 1, size = 14, part = "header") |>
...
```{r within-occupation-analysis}
#| label: within-occupation-analysis
#| cache: true
...
within_occupation_analysis <- defensive_analysis_data |>
  ...
  dplyr::mutate(
    occupation_simplified = dplyr::case_when(
      base::substr(OCCP, 1, 1) == "1" ~ "Management_Professional",
      base::substr(OCCP, 1, 1) == "2" ~ "Professional_Services", 
      base::substr(OCCP, 1, 1) == "3" ~ "Service_Occupations",
      base::substr(OCCP, 1, 1) == "4" ~ "Sales_Office_Admin",
      base::substr(OCCP, 1, 1) == "5" ~ "Production_Transportation",
      TRUE ~ "Other_Occupations"
    ),
  ...
  dplyr::filter(
    ...
    occupation_simplified %in% c("Management_Professional", "Professional_Services", "Sales_Office_Admin")
  )
...
occupation_income_patterns <- within_occupation_analysis |>
  ...
  dplyr::summarise(
    sample_size = dplyr::n(),
    median_income = stats::median(per_capita_household_income_defensive, na.rm = TRUE),
    median_individual_wages = stats::median(individual_wage_income_defensive, na.rm = TRUE),
    average_hours_worked = base::mean(WKHP, na.rm = TRUE),
    .groups = "drop"
  ) |>
...
::: {.content-visible when-format="html"}
```{r occupation-income-patterns-dt}
#| label: occupation-income-patterns-dt
...
::: {.content-visible unless-format="html"}
```{r occupation-income-patterns-ft}
#| label: occupation-income-patterns-ft

flextable::flextable(occupation_income_patterns) |>
  flextable::set_caption(caption = "DIAGNOSTIC: Income Patterns by Occupation and Race") |>
  flextable::add_header_lines(values = "Occupation Income Patterns") |>
  flextable::color(i = 1, color = "blue", part = "header") |>
  flextable::italic(i = 1, part = "header") |>
  flextable::align(i = 1, align = "center", part = "header") |>
  flextable::fontsize(i = 1, size = 14, part = "header") |>
  flextable::bg(i = 1, bg = "white", part = "header") |>
  flextable::bg(i = 2, bg = "palegreen", part = "header") |>
  flextable::colformat_double(j = c("median_income", "median_individual_wages"), big.mark = ",", prefix = "$", digits = 0) |>
  ftExtra::colformat_md() |>
  flextable::autofit()
```
:::

```{r structural-discrimination-test-data}
#| label: structural-discrimination-test-data
...
structural_discrimination_test <- occupation_income_patterns |>
  ...
  dplyr::summarise(
    racial_groups_in_occupation = dplyr::n(),
    income_range_same_occupation = base::ifelse(dplyr::n() >= 2, base::max(median_income) - base::min(median_income), 0),
    wage_range_same_occupation = base::ifelse(dplyr::n() >= 2, base::max(median_individual_wages) - base::min(median_individual_wages), 0),
    hours_range_same_occupation = base::ifelse(dplyr::n() >= 2, base::max(average_hours_worked) - base::min(average_hours_worked), 0),
    structural_discrimination_evidence = base::ifelse(income_range_same_occupation > 15000, 
                                               "POSSIBLE - Large income gaps within same occupation",
                                               "MINIMAL - Small income gaps within same occupation"),
    .groups = "drop"
  ) |>
  dplyr::filter(racial_groups_in_occupation >= 2)
```

::: {.content-visible when-format="html"}
```{r structural-discrimination-test-dt}
#| label: structural-discrimination-test-dt
...
::: {.content-visible unless-format="html"}
```{r structural-discrimination-test-ft}
#| label: structural-discrimination-test-ft

flextable::flextable(structural_discrimination_test) |>
  flextable::set_caption(caption = "Same-Occupation Analysis: Testing for Structural Discrimination") |>
  flextable::add_header_lines(values = "Structural Discrimination Test") |>
  flextable::color(i = 1, color = "blue", part = "header") |>
  flextable::italic(i = 1, part = "header") |>
  flextable::align(i = 1, align = "center", part = "header") |>
  flextable::fontsize(i = 1, size = 14, part = "header") |>
  flextable::bg(i = 1, bg = "white", part = "header") |>
  flextable::bg(i = 2, bg = "palegreen", part = "header") |>
  flextable::colformat_double(j = c("income_range_same_occupation", "wage_range_same_occupation"), big.mark = ",", prefix = "$", digits = 0) |>
  ftExtra::colformat_md() |>
  flextable::autofit()
```
:::

```{r occupation-controlled-patterns}
#| label: occupation-controlled-patterns
...
occupation_controlled_patterns <- within_occupation_analysis |>
  ...
  dplyr::summarise(
    high_performer_income = base::ifelse(base::any(success_level_occupation == "High_Performer_Occupation"),
                                  stats::median(per_capita_household_income_defensive[success_level_occupation == "High_Performer_Occupation"], na.rm = TRUE),
                                  NA),
    low_performer_income = base::ifelse(any(success_level_occupation == "Low_Performer_Occupation"),
                                 median(per_capita_household_income_defensive[success_level_occupation == "Low_Performer_Occupation"], na.rm = TRUE),
                                 NA),
    within_group_gap_occupation_controlled = base::ifelse(!base::is.na(high_performer_income) & !base::is.na(low_performer_income), 
                                                   high_performer_income - low_performer_income, NA),
    .groups = "drop"
  ) |>
  dplyr::filter(!base::is.na(within_group_gap_occupation_controlled), within_group_gap_occupation_controlled > 0) |>
  dplyr::group_by(occupation_simplified) |>
  dplyr::summarise(
    racial_groups_with_both_levels = dplyr::n(),
    average_within_group_gap_occupation = base::mean(within_group_gap_occupation_controlled, na.rm = TRUE),
    .groups = "drop"
  ) |>
...
occupation_defense_results <- structural_discrimination_test |>
  ...
  dplyr::mutate(
    within_to_between_ratio_occupation = base::ifelse(!base::is.na(average_within_group_gap_occupation) & income_range_same_occupation > 0,
                                               round(average_within_group_gap_occupation / income_range_same_occupation, 1),
                                               NA),
    defense_conclusion = dplyr::case_when(
      !base::is.na(within_to_between_ratio_occupation) & within_to_between_ratio_occupation > 2 ~ "STRONG DEFENSE - Within-group > between-group even in same occupation",
      !base::is.na(within_to_between_ratio_occupation) & within_to_between_ratio_occupation > 1.5 ~ "MODERATE DEFENSE - Pattern persists with occupation controls",
      !base::is.na(within_to_between_ratio_occupation) ~ "WEAK DEFENSE - Structural factors may be important",
...
::: {.content-visible when-format="html"}
```{r occupation-defense-results-dt}
#| label: occupation-defense-results-dt
...
::: {.content-visible unless-format="html"}
```{r occupation-defense-results-ft}
#| label: occupation-defense-results-ft

flextable::flextable(occupation_defense_results) |>
  flextable::set_caption(caption = "Structural Racism Defense: Within-Occupation Pattern Analysis") |>
  flextable::add_header_lines(values = "Occupation Defense Results") |>
  flextable::color(i = 1, color = "blue", part = "header") |>
  flextable::italic(i = 1, part = "header") |>
  flextable::align(i = 1, align = "center", part = "header") |>
  flextable::fontsize(i = 1, size = 14, part = "header") |>
  flextable::bg(i = 1, bg = "white", part = "header") |>
  flextable::bg(i = 2, bg = "palegreen", part = "header") |>
  flextable::colformat_double(j = c("income_range_same_occupation", "average_within_group_gap_occupation"), big.mark = ",", prefix = "$", digits = 0) |>
  ftExtra::colformat_md() |>
  flextable::autofit()
```
:::
...
educational_pathway_patterns <- educational_pathway_analysis |>
  ...
  dplyr::summarise(
    sample_size = dplyr::n(),
    median_income = stats::median(per_capita_household_income_defensive, na.rm = TRUE),
    employment_rate = base::mean(ifelse(employment_stability_measure == "Stable_Full_Time_Employment", 1, 0), na.rm = TRUE),
    .groups = "drop"
  ) |>
...
educational_institution_effects <- educational_pathway_patterns |>
  ...
  dplyr::summarise(
    racial_groups_present = dplyr::n(),
    income_range_same_education_level = base::max(median_income) - min(median_income),
    mean_income_education_level = base::mean(median_income),
    education_explains_differences = base::ifelse(income_range_same_education_level < 10000,
...
::: {.content-visible when-format="html"}
```{r educational-institution-effects-dt}
#| label: educational-institution-effects-dt
...
::: {.content-visible unless-format="html"}
```{r educational-institution-effects-ft}
#| label: educational-institution-effects-ft

flextable::flextable(educational_institution_effects) |>
  flextable::set_caption(caption = "Educational Institution Analysis: Do Different Pathways Explain Racial Patterns?") |>
  flextable::add_header_lines(values = "Educational Institution Effects") |>
  flextable::color(i = 1, color = "blue", part = "header") |>
  flextable::italic(i = 1, part = "header") |>
  flextable::align(i = 1, align = "center", part = "header") |>
  flextable::fontsize(i = 1, size = 14, part = "header") |>
  flextable::bg(i = 1, bg = "white", part = "header") |>
  flextable::bg(i = 2, bg = "palegreen", part = "header") |>
  flextable::colformat_double(j = c("income_range_same_education_level", "mean_income_education_level"), big.mark = ",", prefix = "$", digits = 0) |>
  ftExtra::colformat_md() |>
  flextable::autofit()
```
:::

```{r education-controlled-within-between}
#| label: education-controlled-within-between
...
education_controlled_within_between <- educational_pathway_analysis |>
  ...
  dplyr::summarise(
    high_income_median = stats::median(per_capita_household_income_defensive[success_level_education_pathway == "High_Income_Educational_Path"], na.rm = TRUE),
    low_income_median = stats::median(per_capita_household_income_defensive[success_level_education_pathway == "Low_Income_Educational_Path"], na.rm = TRUE),
    within_group_gap_education_controlled = high_income_median - low_income_median,
    .groups = "drop"
  ) |>
  dplyr::filter(!base::is.na(within_group_gap_education_controlled), within_group_gap_education_controlled > 0) |>
  dplyr::group_by(educational_institution_proxy) |>
  dplyr::summarise(
    average_within_group_gap_education = base::mean(within_group_gap_education_controlled, na.rm = TRUE),
    .groups = "drop"
  )
...
education_defense_results <- educational_institution_effects |>
  ...
  dplyr::mutate(
    within_to_between_ratio_education = base::round(average_within_group_gap_education / income_range_same_education_level, 1),
...
::: {.content-visible when-format="html"}
```{r education-defense-results-dt}
#| label: education-defense-results-dt
...
::: {.content-visible unless-format="html"}
```{r education-defense-results-ft}
#| label: education-defense-results-ft

flextable::flextable(education_defense_results) |>
  flextable::set_caption(caption = "Educational Institution Defense: Pathway-Controlled Analysis Results") |>
  flextable::add_header_lines(values = "Education Defense Results") |>
  flextable::color(i = 1, color = "blue", part = "header") |>
  flextable::italic(i = 1, part = "header") |>
  flextable::align(i = 1, align = "center", part = "header") |>
  flextable::fontsize(i = 1, size = 14, part = "header") |>
  flextable::bg(i = 1, bg = "white", part = "header") |>
  flextable::bg(i = 2, bg = "palegreen", part = "header") |>
  flextable::colformat_double(j = c("income_range_same_education_level", "average_within_group_gap_education"), big.mark = ",", prefix = "$", digits = 0) |>
  ftExtra::colformat_md() |>
  flextable::autofit()
```
:::
...
bootstrap_sample_data <- defensive_analysis_data |>
  dplyr::filter(
    AGEP >= 30 & AGEP <= 50,  # Prime working years
    !base::is.na(per_capita_household_income_defensive),
    !base::is.na(racial_category_defensive)
  ) |>
  dplyr::mutate(
...
calculate_within_between_ratio <- function(data, indices) {
  ...
  high_performers <- bootstrap_data |>
    ...
    dplyr::summarise(median_income = stats::median(per_capita_household_income_defensive, na.rm = TRUE), .groups = "drop")
  ...
  within_groups <- bootstrap_data |>
    ...
    dplyr::filter(!base::is.na(within_gap))
...
base::set.seed(123)  # For reproducible bootstrap results
...
bootstrap_confidence_intervals <- tryCatch({
  ...
  data.frame(
    Original_Ratio = base::round(bootstrap_results$t0, 2),
    Bootstrap_Mean = base::round(mean(bootstrap_results$t, na.rm = TRUE), 2),
    Lower_95_CI = base::round(boot_ci$percent[4], 2),
    Upper_95_CI = base::round(boot_ci$percent[5], 2),
    Sample_Size = base::nrow(bootstrap_sample_data),
    Bootstrap_Replications = base::length(bootstrap_results$t[!base::is.na(bootstrap_results$t)]),
    Confidence_Interpretation = base::ifelse(boot_ci$percent[4] > 1.5, 
...
::: {.content-visible when-format="html"}
```{r bootstrap-confidence-intervals-dt}
#| label: bootstrap-confidence-intervals-dt
...
::: {.content-visible unless-format="html"}
```{r bootstrap-confidence-intervals-ft}
#| label: bootstrap-confidence-intervals-ft

flextable::flextable(bootstrap_confidence_intervals) |>
  flextable::set_caption(caption = "Bootstrap Confidence Interval Analysis: Statistical Reliability Test") |>
  flextable::add_header_lines(values = "Bootstrap Confidence Intervals") |>
  flextable::color(i = 1, color = "blue", part = "header") |>
  flextable::italic(i = 1, part = "header") |>
  flextable::align(i = 1, align = "center", part = "header") |>
  flextable::fontsize(i = 1, size = 14, part = "header") |>
...
```{r sample-size-adequacy-data}
#| label: sample-size-adequacy-data
...
::: {.content-visible when-format="html"}
```{r sample-size-adequacy-dt}
#| label: sample-size-adequacy-dt
...
::: {.content-visible unless-format="html"}
```{r sample-size-adequacy-ft}
#| label: sample-size-adequacy-ft

flextable::flextable(sample_size_adequacy) |>
  flextable::set_caption(caption = "Sample Size Adequacy Assessment by Group") |>
  flextable::add_header_lines(values = "Sample Size Adequacy") |>
  flextable::color(i = 1, color = "blue", part = "header") |>
  flextable::italic(i = 1, part = "header") |>
  flextable::align(i = 1, align = "center", part = "header") |>
  flextable::fontsize(i = 1, size = 14, part = "header") |>
...
calculate_ratio_outlier_sensitivity <- function(data, outlier_percentile_cutoff) {
  ...
  return(list(
    ...
    sample_size = base::nrow(outlier_filtered_data),
...
for(cutoff in outlier_cutoff_levels) {
  ...
  if(!is.null(result) && !base::is.na(result$ratio)) {
    outlier_sensitivity_results <- rbind(outlier_sensitivity_results, data.frame(
      Outlier_Removal_Level = base::paste0("Keep ", cutoff, "% of data"),
      ...
      Within_to_Between_Ratio = base::round(result$ratio, 2),
      Pattern_Stability = base::ifelse(result$ratio > 1.5, "STABLE - Pattern persists", "UNSTABLE - Pattern weakens")
    ))
  }
}
```
...
::: {.content-visible when-format="html"}
```{r outlier-sensitivity-results-dt}
#| label: outlier-sensitivity-results-dt
...
::: {.content-visible unless-format="html"}
```{r outlier-sensitivity-results-ft}
#| label: outlier-sensitivity-results-ft

flextable::flextable(outlier_sensitivity_results) |>
  flextable::set_caption(caption = "Outlier Sensitivity Analysis: Pattern Stability with Different Data Restrictions") |>
  flextable::add_header_lines(values = "Outlier Sensitivity Results") |>
  flextable::color(i = 1, color = "blue", part = "header") |>
  flextable::italic(i = 1, part = "header") |>
  flextable::align(i = 1, align = "center", part = "header") |>
  flextable::fontsize(i = 1, size = 14, part = "header") |>
...
```{r outlier-sensitivity-verdict-data}
#| label: outlier-sensitivity-verdict-data

# Overall outlier sensitivity conclusion
outlier_sensitivity_verdict <- base::data.frame(
  Criticism_Tested = "Outliers Drive All Results",
  Stability_Across_Restrictions = base::ifelse(
    base::sum(outlier_sensitivity_results$Within_to_Between_Ratio > 1.5, na.rm = TRUE) >= 3,
    "STABLE - Pattern consistent across outlier removal levels",
    "UNSTABLE - Pattern depends on outliers"
  ),
  Ratio_Range = base::paste0(base::min(outlier_sensitivity_results$Within_to_Between_Ratio, na.rm = TRUE), " to ", 
                      base::max(outlier_sensitivity_results$Within_to_Between_Ratio, na.rm = TRUE)),
  Defense_Verdict = base::ifelse(
    base::sum(outlier_sensitivity_results$Within_to_Between_Ratio > 1.5, na.rm = TRUE) >= 3,
...
::: {.content-visible when-format="html"}
```{r outlier-sensitivity-verdict-dt}
#| label: outlier-sensitivity-verdict-dt
...
::: {.content-visible unless-format="html"}
```{r outlier-sensitivity-verdict-ft}
#| label: outlier-sensitivity-verdict-ft

flextable::flextable(outlier_sensitivity_verdict) |>
  flextable::set_caption(caption = "Outlier Sensitivity Defense: Overall Stability Assessment") |>
  flextable::add_header_lines(values = "Outlier Sensitivity Verdict") |>
  flextable::color(i = 1, color = "blue", part = "header") |>
  flextable::italic(i = 1, part = "header") |>
  flextable::align(i = 1, align = "center", part = "header") |>
  flextable::fontsize(i = 1, size = 14, part = "header") |>
...
defensive_test_summary <- base::data.frame(
  ...
  Defense_Strength = c(
    base::ifelse(base::exists("generational_consistency_test") && base::grepl("REFUTED", generational_consistency_test$Defense_Verdict), "STRONG", "MODERATE"),
    base::ifelse(base::exists("geographic_consistency_test") && base::grepl("REFUTED", geographic_consistency_test$Defense_Verdict), "STRONG", "MODERATE"),
    base::ifelse(base::exists("occupation_defense_results") && base::sum(base::grepl("STRONG", occupation_defense_results$defense_conclusion)) > 0, "STRONG", "MODERATE"),
    base::ifelse(base::exists("education_defense_results") && base::sum(base::grepl("STRONG", education_defense_results$education_defense_conclusion)) > 0, "STRONG", "MODERATE"),
    base::ifelse(base::exists("bootstrap_confidence_intervals") && base::grepl("RELIABLE", bootstrap_confidence_intervals$Confidence_Interpretation), "STRONG", "MODERATE"),
    base::ifelse(base::exists("outlier_sensitivity_verdict") && base::grepl("REFUTED", outlier_sensitivity_verdict$Defense_Verdict), "STRONG", "MODERATE")
  ),
  Key_Finding = c(
...
::: {.content-visible when-format="html"}
```{r defensive-test-summary-dt}
#| label: defensive-test-summary-dt
...
::: {.content-visible unless-format="html"}
```{r defensive-test-summary-ft}
#| label: defensive-test-summary-ft

flextable::flextable(defensive_test_summary) |>
  flextable::set_caption(caption = "COMPREHENSIVE TIER 3 DEFENSIVE TESTING SUMMARY") |>
  flextable::add_header_lines(values = "Defensive Test Summary") |>
  flextable::color(i = 1, color = "blue", part = "header") |>
  flextable::italic(i = 1, part = "header") |>
  flextable::align(i = 1, align = "center", part = "header") |>
  flextable::fontsize(i = 1, size = 14, part = "header") |>
...
```{r overall-defensive-assessment-data}
#| label: overall-defensive-assessment-data

# Calculate overall defensive strength
strong_defenses <- base::sum(defensive_test_summary$Defense_Strength == "STRONG", na.rm = TRUE)
total_defenses <- base::nrow(defensive_test_summary)
defense_percentage <- base::round((strong_defenses / total_defenses) * 100, 1)

overall_defensive_assessment <- base::data.frame(
  ...
  Defense_Success_Rate = base::paste0(defense_percentage, "%"),
  Overall_Defensive_Strength = base::ifelse(defense_percentage >= 80, 
                                     "HIGHLY ROBUST - Strong defense against major criticisms",
                                     base::ifelse(defense_percentage >= 60,
                                           "MODERATELY ROBUST - Good defense against most criticisms", 
                                           "LIMITED ROBUSTNESS - Some criticisms not fully addressed")),
  Research_Quality_Level = base::ifelse(defense_percentage >= 80,
  ...
  Policy_Confidence_Level = base::ifelse(defense_percentage >= 80,
...
::: {.content-visible when-format="html"}
```{r overall-defensive-assessment-dt}
#| label: overall-defensive-assessment-dt
...
::: {.content-visible unless-format="html"}
```{r overall-defensive-assessment-ft}
#| label: overall-defensive-assessment-ft

flextable::flextable(overall_defensive_assessment) |>
  flextable::set_caption(caption = "FINAL TIER 3 DEFENSIVE ASSESSMENT") |>
  flextable::add_header_lines(values = "Overall Defensive Assessment") |>
  flextable::color(i = 1, color = "blue", part = "header") |>
  flextable::italic(i = 1, part = "header") |>
  flextable::align(i = 1, align = "center", part = "header") |>
  flextable::fontsize(i = 1, size = 14, part = "header") |>
